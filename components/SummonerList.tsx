...
export default function SummonerList({ 
  ...
  return (
    <div className="mb-6 sm:mb-8 md:mb-10">
      <ul className="flex flex-row items-center justify-start gap-2 sm:gap-3 md:gap-4 overflow-x-auto pb-2 scrollbar-hide -mx-1 px-1">
        ...
        {Array.from({ length: emptySlots }).map((_, index) => (
          <li 
            key={`empty-${index}`}
            className="flex flex-col gap-2 items-center justify-center flex-shrink-0"
          >
            ...
            <span className="text-sm sm:text-base text-zinc-400 dark:text-zinc-600">
              빈 슬롯
            </span>
          </li>
        ))}
